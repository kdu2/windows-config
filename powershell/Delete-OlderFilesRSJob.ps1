...
# delete files
$files = Get-ChildItem -Path $path -File -Exclude "*.ps1" -Recurse | Where-Object { $_.CreationTime -lt $limit } #| foreach { Write-Host "deleting $($_.name)"; Remove-Item $_ -Force }
...
$folders = Get-ChildItem -Path $path -Directory -Recurse | Where-Object { (Get-ChildItem -Path $_.FullName -Recurse -File) -eq $null } #| foreach ( Write-Host "deleting $($_.name)"; Remove-Item $_ -Force -Recurse }
